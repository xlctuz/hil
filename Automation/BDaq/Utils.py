...
def toBaudrate(value):
    value = int(value)

    for x in Baudrate:
        if x.value == value:
            return x
    return Baudrate.Baudrate2000KHz

def toCodingType(value):
    value = int(value)

    for x in CodingType:
        if x.value == value:
            return x
    return CodingType.BinaryCode

def toErrorRetType(value):
    value = int(value)

    for x in ErrorRetType:
        if x.value == value:
            return x
    return ErrorRetType.Current
