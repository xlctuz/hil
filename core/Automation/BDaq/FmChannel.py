...
class FmChannel(object):
    ...
    @property
    def timeUp(self):
        return TFmChannel.getTimeUp(self._nativeFmChanObj)

    @timeUp.setter
    def timeUp(self, value):
        if not isinstance(value, (float, int)):
            raise TypeError("a float is required")
        ret = ErrorCode.lookup(TFmChannel.setTimeup(self._nativeFmChanObj, value))
        if BioFailed(ret):
            raise ValueError('set timeUp is failed, the error code is 0x%X' % (ret.value))
