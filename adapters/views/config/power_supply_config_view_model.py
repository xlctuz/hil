...
class PowerSupplyChannelConfigViewModel(QObject):
...
class PowerSupplyConfigViewModel(QObject):
    resourceNameChanged = Signal()
    baudRateChanged = Signal()

    def __init__(self, usecases: UseCases, power_supply_data, parent=None):
        super().__init__(parent)
        ...
        if self._power_supply_data:
            ...
            for ch_data in sorted_channels:
                self._channels.append(PowerSupplyChannelConfigViewModel(ch_data, self))
        # ensure 3 channels exist for QML binding
        while len(self._channels) < 3:
            self._channels.append(PowerSupplyChannelConfigViewModel(None, self))
